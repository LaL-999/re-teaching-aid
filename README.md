...
| 需求工程流水线 | 八车间闭环 + **一句话直达驾驶舱**；各车间产物可一键流转下游、就地审核优化、**版本树迭代/回滚/对比/标记最佳**、持久保存与清空 | 师生共用 |
| ├ 🚀 流水线驾驶舱 | 一句话想法 → 自动贯通 ①→③→④ 生成；总览各车间状态与进度 | 师生共用 |
...
| ├ ⑤ i\* 目标建模 | 需求 → piStar 2.0 结构化 JSON，可导入 piStar 工具渲染 | 师生共用 |
| ├ ⑥ UML 建模与预览 | 需求 → PlantUML（用例 / 活动 / 类图）+ 在线编译预览，改码即重编 | 师生共用 |
| ├ ⑦ SRS 规格说明书 | **素材驱动**：上游需求素材一键成稿（自动提炼项目名 / 功能点，无需手填），GB/T 9385 结构，导出 MD/HTML | 师生共用 |
| ├ ⑧ 需求追踪矩阵 | 具体需求 + SRS → 需求 ↔ 系统 追踪链表，覆盖状态一目了然，导出 CSV | 师生共用 |
| └ 🎛 提示词工坊 | 教师在前端编辑每个阶段 AI 的「输出格式」提示词，即存即用（落盘后端），可随时重置 | 教师编辑 / 学生只读 |

---

## 需求工程流水线（闭环）

8 个「车间」按生产顺序串联，构成可闭环的需求工程产线，并由「流水线驾驶舱」统一驱动：
...
- **一句话直达**：在「流水线驾驶舱」输入一句话想法，一键自动贯通 ①产品概要 → ③具体需求 → ④需求分析与审查，端到端成形。
- **数据流转**：每个车间产物下方有「发送到下一车间」按钮，把产物写入下游输入并跳转，无需手工复制粘贴；轨道导航用 ✓ 显示各阶段是否已产出。
- **就地优化**：文本类车间均有「审核并优化」，调用大模型对当前产物做评审改写；④ 本身即「审查＋优化」环节。
- **产物版本树**：每个车间产物可存版本，支持回滚 / 标记最佳 / 删除 / 与当前对比，便于不断迭代后选出最佳一版。
- **可编辑输出格式**：在「提示词工坊」教师可改写每个阶段 AI 的输出格式提示词，即存即用（落盘 `server/data/prompt-templates.json`，后端下次生成即采用），可随时重置。
- **SRS 素材驱动**：⑦ 仅凭上游需求素材即可成稿，AI 自动提炼项目名 / 功能点，无需手填。
- **需求追踪**：⑧ 将具体需求与 SRS 对应成「需求 ↔ 系统」追踪矩阵，覆盖状态一目了然，可导出 CSV。
- **持久与清空**：所有车间的输入与产物存于可持久化工作台（`store/workbenchStore.ts`，localStorage，按用户隔离），**切换车间 / 刷新页面均不丢失**；每个车间提供「清空」。
